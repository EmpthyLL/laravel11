  ...
  <main>
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 flex flex-col lg:flex-row gap-6">
      ...
      <div class="border border-yellow-200 shadow-lime-100 hover:shadow-lime-200 transition-shadow shadow-lg hover:shadow-xl p-6 rounded-lg bg-lime-100 flex flex-col items-center gap-6 h-max">
        <img src="{{ asset('img/img_' . (($comments[0]->users->id - 1) % 25 + 1) . '.jpg') }}" class="rounded-full w-[150px] p-1 border-2 border-yellow-200 shadow-lg bg-yellow-100" alt="Profile Picture">
        <div class="text-center">
          <h2 class="text-3xl font-bold tracking-tight text-emerald-900">{{ $comments[0]->users['name'] }}</h2>
          <h3 class="text-2xl tracking-tight text-emerald-400">{{ $comments[0]->users['email'] }}</h3>
          <h3 class="text-2xl tracking-tight text-emerald-400 flex justify-center gap-4">
            <span>Comments:</span><span class="font-bold">{{ count($comments) }}</span>
          </h3>
        </div>
      </div>
